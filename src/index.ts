...
const run = async (): Promise<void> => {
  try {
    ...
    try {
      ...
      throwError(
        error,
        'Failed to retrieve url from action. See configuration for instructions on how to add covergeEndpoint to action.'
      );
    }
    ...
    if (context.payload.pull_request && prData?.coverage) {
