...
const run = async () => {
    try {
        const ignoredUsers = (0, core_1.getInput)('ignoredUsers')
            .split(',')
            .map((user) => user.trim());
        if (ignoredUsers.includes(github_1.context.payload.pull_request?.user.login)) {
            console.log(`Skipping the action because the pull request is created by ${github_1.context.payload.pull_request?.user.login}`);
            return;
        }
